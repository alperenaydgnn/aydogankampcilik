...
export function CartDrawer() {
  const { isOpen, close, items, setQty, remove, subtotal, combo, total, count, hasNumericPrices, clear } = useCart();
  ...
  return (
    <>
                        ...
                        <div className="flex-1 min-w-0 flex flex-col">
                          ...
                          <div className="mt-auto flex items-center justify-between pt-3">
                            <div className="inline-flex items-center border border-foreground/15 rounded-full overflow-hidden">
                              ...
                              <span className="px-3 text-xs font-semibold tabular-nums">{item.qty}</span>
                              <button
                                onClick={() => setQty(item.id, item.qty + 1)}
      ...
      <CheckoutWizard open={checkoutOpen} onClose={() => setCheckoutOpen(false)} />
    </>
  );
}
