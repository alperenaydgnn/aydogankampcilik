...
export function buildProductMessage(
  ...
  if (product.price_numeric) {
    lines.push(`💰 Fiyat: ${product.price_label}`);
  }
...
export function buildSearchMessage(query: string, categoryName?: string): string {
  ...
  if (categoryName) {
    lines.push(`*${categoryName}* kategorisinde ürün arıyorum.`);
  } else {
    lines.push('Kamp ve balık malzemeleri hakkında bilgi almak istiyorum.');
  }

  if (query.trim()) {
    lines.push('', `🔍 Aradığım: *"${query.trim()}"*`);
  }
